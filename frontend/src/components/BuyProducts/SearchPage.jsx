...
const SearchPage = () => {
  ...
  try {
    ...
    const favouriteProductIds = response.data.favourites.map(p => p.id);
    ...
    setError(err.response?.data?.message || 'Failed to fetch favourites. Please try again.');
  } finally {
    setLoading(false);
  }
};
  ...
  const updateFavourite = async (productId) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/products/favourites/${productId}`, {
        userId: userDetails.ID,
      });
      if (!response.data.success) {
        setError("Failed to update favourites on server");
        setFavourites(prev =>
          isFav ? [...prev, productId] : prev.filter(id => id !== productId)
        );
      }
    } catch (error) {
      console.error('Failed updating favourites:', error);
  ...
  if (debouncedSearchTerm.trim() !== '') {
    ...
    const hits = fuse.search(debouncedSearchTerm).map(r => r.item);
  ...
  if (debouncedSearchTerm.trim() !== '') {
    ...
    const fuseHits = fuse.search(debouncedSearchTerm).map(r => r.item);
    const sv = buildTfidf([{ id: 'q', name: debouncedSearchTerm }])['q'] || {};
  ...
  if (error) return <div className="min-h-screen flex flex-col items-center justify-center gap-4">
    ...
    <button onClick={fetchProducts} className="bg-blue-500 text-white px-4 py-2 rounded">Retry</button>
  </div>;
              ...
              {filtered.map(p => {
                ...
                return (
                  <div key={p.id} className="bg-white rounded-2xl shadow hover:shadow-lg transition overflow-hidden group">
                    <div className="relative cursor-pointer" onClick={() => handleClick(p.id)}>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <button
                          ...
                          className="flex-1 bg-gradient-to-r from-[#f3b15c] to-[#ed8888] text-white py-2 rounded-lg hover:opacity-90"
                        >
                          View Details
                        </button>

                        <button
                          onClick={() => updateFavourite(p.id)}
                        ...
                        >
                          {favourites.includes(p.id) ? <FaHeart /> : <FaRegHeart />}
                        </button>
                      </div>
                    </div>
                  </div>
